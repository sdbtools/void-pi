...
syslinux_install_efi(BD, RD) :-
	...
	% os_shell2([chroot, RD, efibootmgr, '--create', oo(disk, BD), oo(part, 1), oo(loader, '/EFI/syslinux/syslinux.efi'), oo(label, '"Syslinux"'), '--unicode']),
	tui_progressbox_safe([chroot, RD, efibootmgr, '--create', oo(disk, BD), oo(part, 1), oo(loader, '/EFI/syslinux/syslinux.efi'), oo(label, '"Syslinux"'), '2>&1'], 'efibootmgr', [sz([12, 80])]),
	true.
