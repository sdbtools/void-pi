...
st_root_fs(TL, FS) :-
	memberchk(state(root_fs, ctx_rfs(_PTT, FS)), TL),
	!.
st_root_fs(_TL, ext4).

st_part_tmpl(TL, PTN) :-
	memberchk(state(make_part_tmpl, ctx_part4(_PTT, _FS, PTN, _D4L)), TL),
	!.
...
st_bootloader(TL, B) :-
	memberchk(state(bootloader, ctx_bl(B)), TL).
...
st_retrieve_ctx(state(root_fs, ctx_rfs(_PTT, FS)), L, [fs(FS)|L]) :- !.
st_retrieve_ctx(state(make_part_tmpl, ctx_part4(_PTT, _FS, PTN, _D4L)), L, [part_tmpl(PTN)|L]) :- !.
